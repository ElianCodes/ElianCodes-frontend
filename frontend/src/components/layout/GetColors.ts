...
const init = () => {
	...
	if (document.querySelector('html').getAttribute('class') != undefined && document.querySelector('html').getAttribute('class').includes('index-bg')){
		document.querySelector('html').style.background = `linear-gradient(90deg, #FFF 50%, ${color.code} 50%)`
	}
}
